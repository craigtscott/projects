...
def harrison_ford
  ...
  Movie.select('movies.id, movies.title').joins(:castings).joins(:actors).where("castings.ord != 1 AND castings.actor_id = 6").uniq
...
def biggest_cast
  ...
  Movie.select('movies.id, movies.title').joins(:actors).group('movies.id').order('COUNT(actors.name) DESC').limit(3)
...
def directed_by_one_of(them)
  ...
  # dir_ids = Actor.select('id').where("actors.name IN (?)", them)
  # Movie.select('movies.id, movies.title').joins(:actors).where("movies.director_id IN (?)", dir_ids).uniq
